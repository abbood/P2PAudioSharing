...
-(BOOL)openLocalFileStream
{
    @synchronized(self)
	{
        ...
        Boolean streamSupportsAsyncNot = CFReadStreamSetClient(
                              stream,
                              kCFStreamEventHasBytesAvailable | kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered | kCFStreamEventCanAcceptBytes | kCFStreamEventOpenCompleted,
	...
	eventType:(CFStreamEventType)eventType
{
	...
	{
        NSLog(@"handleReadFromStream: The stream has bytes to be read.");
		...
		@synchronized(self)
		{
			...
			//
            totalBytesHandeled += kAQBufSize;
            NSLog(@"we are handling %d bytes with total %lu", kAQBufSize, totalBytesHandeled);
 ...
 //   NSLog(@"inside enqueue buffer");
	@synchronized(self)
	{
     ...
     /*   NSData *bufContent = [NSData dataWithBytes:fillBuf->mAudioData length:fillBuf->mAudioDataByteSize];
        NSLog(@"we are enqueuing buffer (length: %lu) %@",fillBuf->mAudioDataByteSize, bufContent);
        NSLog(@"\n\n\n");            
        NSLog(@":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::");*/
		
		if (packetsFilled)
		{
                        //NSLog(@":::::: we are enqueuing buffer");
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			if (buffersUsed == kNumAQBufs - 1)
			{
				...
				{
                              //  NSLog(@"start queue thread name %@", [NSThread currentThread].name);
                    
                     NSLog(@"WE ARE STARTING AND PRIMING QUEUE");
        ...
        //NSLog(@"now we are going to next buffer with fill buff index being: %d",fillBufferIndex);
	...
	{
      //  NSLog(@"WE ARE BEING BLOOOOCKED on bufindex %d", fillBufferIndex);
	...
	if (inPacketDescriptions)
	{
		...
		for (int i = 0; i < inNumberPackets; ++i)
		{
			...
			@synchronized(self)
			{
				...
				{
                    NSLog(@"oops! packet size %lld is bigger than buf size %d",packetSize, kAQBufSize);
...
-(void)startReader
{
    ...
    while (!currentSong) {                        
        //wait
        pthread_cond_wait(&queueBufferReadyCondition, &queueBuffersMutex);
        //NSLog(@"we are currently waiting for current songto show up");
    }
    pthread_mutex_unlock(&queueBuffersMutex);
    
    @synchronized(self)
    {
        //NSLog(@"we're done waiting! now we wil read item withcurrent song %@", currentSong);
        AudioItem * item = [audioPool.pool objectForKey:currentSong];

        //NSLog(@"we have fetched item");
        if (!item)
        {
            //NSLog(@"cannot find song in container");
            return;
        }

        while ([item.data length] > 0) {
            //NSLog(@"we are looping as long as there is length in item.data");
            [self doRead:item];
            if (isSongChanged) {
                //NSLog(@"song has changed! time to change item");
                item = [audioPool.pool objectForKey:currentSong];
            }
        }
    }
...
-(void)doRead:(AudioItem *)item
{    
    NSData *dataRead;
         
    @synchronized(self)
    {        
        //NSLog(@"we are doReading.. ");
        dataRead = [NSData dataWithBytes:[item.data bytes] length:MAX_PACKET_SIZE];        
        [audioPool purgeReadData:item length:MAX_PACKET_SIZE];
        
        [self handleContainerPackets:[dataRead bytes] numberBytes:MAX_PACKET_SIZE];
    }        
}
...
-(void)handleContainerPackets:(const void *)inInputData
                  numberBytes:(UInt32)inNumberBytes
{    
    //NSLog(@"handlecontainerpacket  thread name %@", [[NSThread currentThread] name]);
    //NSLog(@"WE ARE ABOUT TO HANDLE CONTAINER PACKETS");
    
    size_t incomingDataOffset = 0;
    while(inNumberBytes) {
      //  NSLog(@"inside the innumberbytes loop");
        ...
        @synchronized(self)
        {
         //   NSLog(@"inside the syncrhoized part");
            bufSpaceRemaining = kAQBufSize - bytesFilled;
            size_t copySize;    
            ...
            // copy data to the audio queue buffer
            AudioQueueBufferRef fillBuf = audioQueueBuffer[fillBufferIndex];
         //   NSData * temp = [NSData dataWithBytes:inInputData length:inNumberBytes];
            memcpy((char*)fillBuf->mAudioData + bytesFilled, 
                   (const char*)(inInputData + incomingDataOffset), copySize); 
        ...
        {
       //     NSLog(@"the space inside buffer is not enough! about to nequeue buf");
            [self enqueueBuffer];
        }
